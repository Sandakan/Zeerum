...
fetchData(`/data/tags/`, (data) => {
	data.forEach((x) => {
		if (window.location.pathname.split('/')[1] === 'search') {
			document.title = `ZEERUM \| You searched for ${window.location.pathname.split('/').pop()}`;
		...
		} else if (x.name.toLowerCase() === window.location.pathname.split('/').pop()) {
...
fetchData(`/data/articles/`, (data) => {
	// const redirect = (url) => (window.location = `../articles/${encodeURI(url)}`);
	data.forEach((x) => {
		// console.log(x);
		document.querySelector(
		...
		x.tags.forEach((y) => {
			if (window.location.pathname.split('/').pop() == y.toLowerCase()) {
		...
		if (x.title.toLowerCase().includes(window.location.pathname.split('/').pop())) {
			document.querySelector('.articles-container').innerHTML =
